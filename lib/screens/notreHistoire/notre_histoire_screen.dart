  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text("Notre histoire", style: TextStyle(color: Colors.white)),
      ),
            ...
            child: Column(
              ...
              children: [
                const Center(
                  child: Column(
                    children: [
                      Text("Notre histoire", style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: violetFonce)),
                      SizedBox(height: 8),
                      Text(
                        "Le rideau se lève sur une aventure humaine, artistique et généreuse.",
                        style: TextStyle(fontSize: 16, color: violetFonce),
...
class _ActeWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final texteWidget = Padding(
      padding: EdgeInsets.symmetric(horizontal: isMobile ? 0 : 16, vertical: isMobile ? 12 : 0),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(titre, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: violetFonce)),
          const SizedBox(height: 8),
          Text(texte, style: const TextStyle(fontSize: 14, color: violetFonce, height: 1.5)),
        ],
      ),
    );
    ...
    return Padding(
      ...
      child: isMobile
        ...
        children: imageLeft ? [image, Expanded(child: texteWidget)] : [Expanded(child: texteWidget), image],
      ),
    );
  }
}